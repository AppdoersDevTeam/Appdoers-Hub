  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          {isSent && (
            <a
              ...
              className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 px-3 py-1.5 text-sm text-slate-500 hover:border-blue-600 hover:text-slate-900 transition-colors"
            >
      ...
      {contract.status === 'signed' && (
        <div className="flex items-center gap-4 border-b border-emerald-200 bg-emerald-50 px-6 py-3">
          ...
          <div className="mt-6 border-t border-slate-200 px-4 pt-4 space-y-3">
            ...
            {contract.sent_at && (
              <div>
                ...
                <p className="text-xs text-slate-500">{formatDate(contract.sent_at)}</p>
              </div>
            )}
            {contract.signed_at && (
              <div>
                <p className="text-xs text-slate-500">Signed</p>
                <p className="text-xs text-slate-500">{formatDate(contract.signed_at)}</p>
              </div>
            )}
          </div>
        </div>
            ...
            return (
              <div key={s.id} className="space-y-4">
                ...
                <textarea
                  ...
                  className="w-full rounded-md border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-blue-500 focus:outline-none resize-y leading-relaxed disabled:opacity-60 disabled:cursor-not-allowed"
                  placeholder={`Write the ${s.title.toLowerCase()} clause here…`}
                />
                {contract.status === 'signed' && (
                  <p className="text-xs text-slate-500">This contract has been signed and cannot be edited.</p>
                )}
              </div>
            )
          })}
