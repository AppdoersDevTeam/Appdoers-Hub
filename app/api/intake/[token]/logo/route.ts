...
export async function POST(
  ...
  if (rateLimitHit(`intake-logo:${tokenHash}:${clientIp(req.headers)}`, 20, 60 * 60_000)) {
  ...
  let body: { step?: string; file_name?: string; mime_type?: string; file_size?: number; storage_path?: string }
  try {
    body = await req.json()
  } catch {
  ...
  if (body.step === 'complete') {
    ...
    await service.from('client_intakes').update({ answers, updated_at: new Date().toISOString() }).eq('id', intake.id)
