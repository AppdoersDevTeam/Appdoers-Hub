...
function lookupToken(token: string) {
  const trimmed = token.trim()
  if (!trimmed || trimmed.length > 128) return null
  return hashIntakeToken(trimmed)
}
...
export async function GET(
  ...
  const tokenHash = lookupToken(token)
  if (!tokenHash) {
    return NextResponse.json({ error: 'Invalid link' }, { status: 400 })
  }

  const ip = clientIp(_req.headers)
  if (rateLimitHit(`intake-get:${ip}`, 60, 60_000)) {
    return NextResponse.json({ error: 'Too many requests' }, { status: 429 })
  }

  const service = createAdminClient()
  if (!service) {
    return NextResponse.json({ error: 'Intake is unavailable' }, { status: 500 })
  }

  const { data: intake } = await service
    .from('client_intakes')
    .select('id, status, answers, client_id, clients(company_name, location, website)')
    .eq('token_hash', tokenHash)
    .maybeSingle()

  if (!intake) {
    return NextResponse.json({ error: 'This intake link is invalid.' }, { status: 404 })
  }

  const client = Array.isArray(intake.clients) ? intake.clients[0] : intake.clients
  const answers = mergeIntakeAnswers(intake.answers)
  if (!answers.people.company_name) {
    answers.people.company_name = client?.company_name ?? ''
  }
  if (!answers.people.location) {
    answers.people.location = client?.location ?? ''
  }
  ...
  return NextResponse.json({
    ...
    answers: intake.status === 'sent' && Object.keys((intake.answers as object) ?? {}).length === 0
      ? answers
      : mergeIntakeAnswers(intake.answers).people.company_name
        ? mergeIntakeAnswers(intake.answers)
        : answers,
    defaults: emptyIntakeAnswers(),
  })
}
...
export async function POST(
  ...
  if (rateLimitHit(`intake-post:${tokenHash}:${ip}`, 8, 10 * 60_000)) {
  ...
  if (serialized.length > ANSWERS_MAX_BYTES) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Saved the form but could not update the client record. Appdoers has been notified.' },
      { status: 500 }
    )
  }
