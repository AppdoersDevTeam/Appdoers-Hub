...
export async function notifyIntakeSubmitted(input: {
  ...
  const fontLabel =
    input.answers.brand.font_mode === 'pairing'
      ...
      : input.answers.brand.font_mode === 'custom'
        ...
        : 'Appdoers to choose'
  const launch = input.answers.features.no_deadline
    ? 'No deadline'
    : input.answers.features.launch_date || 'Not set'
  ...
  const blocks = buildSlackAlert({
    ...
    fields: [
      ...
      { label: 'Logo', value: input.answers.brand.logo_path ? 'Uploaded' : input.answers.brand.logo_mode.replaceAll('_', ' ') },
    ],
