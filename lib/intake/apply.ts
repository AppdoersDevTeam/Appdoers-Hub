...
function normalizeDomain(value: string) {
  return value.trim().replace(/^https?:\/\//i, '').replace(/^www\./i, '').split('/')[0].toLowerCase()
}
...
async function upsertContact(
  ...
  if (isPrimary) {
    await service.from('client_contacts').update({ is_primary: false }).eq('client_id', clientId).eq('is_primary', true)
  }
  ...
  if (existing?.id) {
    await service.from('client_contacts').update(payload).eq('id', existing.id)
    return
  }

  await service.from('client_contacts').insert({ client_id: clientId, ...payload, has_portal_access: false })
}
...
async function upsertDomain(service: ServiceClient, clientId: string, answers: IntakeAnswers) {
  ...
  const hosting =
    answers.access.hosting === 'appdoers' ? 'Appdoers' : answers.access.hosting === 'have' ? 'Client-managed' : null
  const notes = [
  ...
  const payload = { domain_name: domainName, registrar, hosting_provider: hosting, dns_notes: notes || null }
  if (existing?.id) {
    await service.from('client_domains').update(payload).eq('id', existing.id)
    return
  }
  await service.from('client_domains').insert({ client_id: clientId, ...payload, ssl_status: 'none', tech_stack: [] })
}
...
async function upsertCredential(
  ...
  await service.from('client_credentials').insert({ client_id: clientId, ...payload })
}
...
function buildBrandKit(answers: IntakeAnswers): ClientBrandKit {
  ...
  return {
    ...
    mood_id: mood?.id ?? answers.brand.mood_id,
  }
}
...
export async function applyIntakeToClient(clientId: string, answers: IntakeAnswers) {
  ...
  await service
    .from('clients')
    .update({
      company_name: answers.people.company_name.trim() || undefined,
      website:
  ...
  await upsertCredential(service, clientId, 'Hosting', answers.access.hosting_username, answers.access.hosting_password)
